    ...
    try:
        ...
        if patron:
            ...
            if cur_app.fetchone()[0] == 0:
                ...
                cur_app.execute("""
                    INSERT INTO users (username, email, role, teacher_name, darajah_name, trno, class_name)
                    VALUES (?, ?, 'student', ?, ?, ?, ?)
                """, (username, f"{username}@placeholder.local", student_name, darajah_name, trno, darajah_name))
