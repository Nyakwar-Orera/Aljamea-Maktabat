...
def create_app():
    ...
    @app.template_filter('hijri_format')
    def hijri_format_filter(value):
        if not value:
            return "N/A"
        from services.koha_queries import get_hijri_date_label
        ...
        if isinstance(value, datetime):
            ...
            try:
                # Common format in this app is YYYY-MM-DD
                d = datetime.strptime(value.split(' ')[0], '%Y-%m-%d').date()
            except:
                return value
        
        if d:
            return get_hijri_date_label(d)
        return str(value)
    ...
    def inject_branch_context():
        """
        Inject branch-aware context into every template.
        Provides current_branch_code, current_branch_cfg, and is_super_admin.
        """
        from flask import session as _session
        from config import Config as _Cfg

        is_super = _session.get("is_super_admin", False) or _session.get("role") == "super_admin"
        branch_code = _session.get("branch_code") or ("AJSN" if not is_super else None)
        branch_cfg = _Cfg.CAMPUS_REGISTRY.get(branch_code, {}) if branch_code else {}

        return {
            "current_branch_code": branch_code,
            "current_branch_cfg":  branch_cfg,
            "current_branch_name": branch_cfg.get("short_name", branch_code or "Global"),
            "current_branch_flag": branch_cfg.get("flag", "🌐"),
            "is_super_admin":      is_super,
            "campus_registry":     _Cfg.CAMPUS_REGISTRY,
            "branch_order":        _Cfg.BRANCH_ORDER,
        }
    ...
    def set_security_headers(response):
        ...
        response.headers["Content-Security-Policy"] = (
            ...
            "img-src 'self' data: https://books.google.com https://via.placeholder.com https://*.jameasaifiyah.org https://images-na.ssl-images-amazon.com; "
            "connect-src 'self';"
        )
        return response
