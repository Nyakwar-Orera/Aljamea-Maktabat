...
import os
import sqlite3  # 🔹 for catching IntegrityError / OperationalError
...
def add_user():
    """
    Add a new user (Admin / HOD / Teacher) with:
    ...
    try:
        ...
        if role == "hod" and not department_name:
            return jsonify(
                success=False,
                error="Department name is required for Heads of Department.",
            )
        if role == "teacher" and not class_name:
            return jsonify(
                success=False,
                error="Class name is required for Class Teachers.",
            )
        ...
        try:
            ...
            if mail:
                ...
                cur2 = conn2.cursor()
                cur2.execute(
                    "SELECT subject, html FROM email_templates WHERE template_key='account_created'"
                )
                row = cur2.fetchone()
                cur2.close()
                conn2.close()
