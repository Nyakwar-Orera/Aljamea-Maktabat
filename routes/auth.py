...
def generate_external_token(username: str, role: str = "teacher") -> str:
    """Generate a signed, time-limited token for external login."""
    s = _get_token_serializer()
    return s.dumps({"username": username, "role": role})


def verify_external_token(token: str, max_age: int = 86400):
    """Verify and decode a signed token. Raises on invalid/expired."""
    s = _get_token_serializer()
    try:
        data = s.loads(token, max_age=max_age)
        return data["username"], data.get("role", "teacher"), None
    except SignatureExpired:
        raise ValueError("Token has expired. Please request a new login link.")
    except BadSignature:
...
def token_login():
    ...
    session["role"] = role
    session["class_name"] = user.get("class_name")
    session["profile_picture"] = user.get("profile_picture", "images/avatar.png")
