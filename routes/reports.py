...
@bp.route("/export/top-stars")
def export_top_stars():
    """Export Top Star Patrons as PDF (Support Global scope for Super Admin)."""
    if not session.get("logged_in") or session.get("role") not in ["admin", "super_admin"]:
        return "Unauthorized", 401
    
    sex = request.args.get("sex", "M").upper()
    scope = request.args.get("scope", "campus") # 'campus' or 'global'
    ay = session.get("selected_ay", "current")
    hijri_year = int(ay) if ay != "current" else None
    limit = request.args.get("limit", 20, type=int)
    
    if scope == "global" and session.get("is_super_admin"):
        from services.branch_queries import get_global_top_students_by_sex
        rows = get_global_top_students_by_sex(sex, limit=limit, hijri_year=hijri_year)
        branch_name = "Global Information Network"
        bc = "GLOBAL"
        
        # Prepare DataFrame for Global
        data = []
        for i, r in enumerate(rows, 1):
            data.append({
                "Rank": i,
                "Campus": r.get("branch_name"),
                "ID": r.get("borrowernumber") or r.get("cardnumber"),
                "Name": r.get("StudentName"),
                "Class": r.get("Class"),
                "Marhala": r.get("Marhala"),
                "Issues": r.get("BooksIssued")
            })
    else:
        # Get branch info for header
        from config import Config
        bc = session.get("branch_code", "AJSN")
        branch_name = Config.CAMPUS_REGISTRY.get(bc, {}).get("short_name", bc)
        
        # Get data
        rows = KQ.get_top_students(limit, None, hijri_year=hijri_year, sex=sex)
    ...
    gender_label = "Male" if sex == "M" else "Female"
    scope_label = "Global" if scope == "global" else "Branch"
    title = f"{scope_label} Star Patrons Report ({gender_label})"
    subtitle = f"Scale: {branch_name} | Academic Year: {ay if ay != 'current' else 'Active'}"
    
    # Summary stats
    summary = {
        "Total Students": len(df),
        "Report Type": f"Top {limit} {gender_label} Readers",
        "Generated By": session.get("username", "Admin")
    }
    ...
    filename = f"{scope_label}_Star_Patrons_{gender_label}_{bc}_{datetime.now().strftime('%Y%m%d')}.pdf"
...
def generate_report():
    ...
    # -------- TAQEEM-WISE (MARKS) --------
        ...
        darajah_val = request.form.get("darajah_value")
        if not darajah_val:
            return jsonify(success=False, html="<p>Please select a specific Darajah for the Marks Report.</p>")
            
        # Teachers/HODs check
        if is_teacher and darajah_val != teacher_darajah:
            return jsonify(success=False, html="<p>Permission denied for this darajah.</p>")

        academic_year = Config.CLEAN_ACADEMIC_YEAR()
        df = taqeem_report_df(darajah_val, academic_year)
        
        if df.empty:
            return jsonify(success=False, html=f"<p>No student marks found for {darajah_val} in AY {academic_year}. Please ensure marks are updated first.</p>")

        html = df.to_html(
            classes="table table-sm table-striped taqeem-table",
            index=False,
            escape=False
        )
