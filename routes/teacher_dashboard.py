...
# --------------------------------------------------
def _darajah_ay_trend(darajah_name: str, hijri_year=None):
    """Get monthly trend data for a darajah"""
    try:
        labels, values = KQ.get_ay_trend_data(darajah_name=darajah_name, hijri_year=hijri_year)
...
def _get_darajah_language_stats(darajah_name: str, start: date, end: date, conn=None):
    """Get issue counts by language for a specific darajah."""
    _conn = conn
    _cur = None
    _close_conn = False
    try:
        ...
        _cur.execute("""
            SELECT 
                CASE 
                    ...
                    WHEN ExtractValue(bmd.metadata, '//datafield[@tag="041"]/subfield[@code="a"]') = 'msl' THEN 'Lisan ud-Dawat'
                    ELSE ExtractValue(bmd.metadata, '//datafield[@tag="041"]/subfield[@code="a"]') 
                END AS language,
                COUNT(s.itemnumber) as count
            FROM statistics s
            JOIN borrowers b ON s.borrowernumber = b.borrowernumber
            JOIN borrower_attributes ba ON b.borrowernumber = ba.borrowernumber 
                AND ba.code IN ('STD', 'Class', 'DAR', 'CLASS', 'CLASS_STD')
            ...
            GROUP BY language
            ORDER BY count DESC
        """, (darajah_name, start, end))
        
        return _cur.fetchall()
    except Exception as e:
        current_app.logger.error(f"Error getting darajah language stats: {e}")
        return []
    finally:
        if _cur: _cur.close()
        if _conn and _close_conn: _conn.close()


def _get_darajah_unique_titles_count(darajah_name: str, start: date, end: date, conn=None):
    """Get count of unique titles issued by a specific darajah."""
    _conn = conn
    _cur = None
    _close_conn = False
    try:
        ...
        _cur.execute("""
            SELECT COUNT(DISTINCT it.biblionumber) as unique_count
            FROM statistics s
            JOIN borrowers b ON s.borrowernumber = b.borrowernumber
            JOIN borrower_attributes ba ON b.borrowernumber = ba.borrowernumber 
                AND ba.code IN ('STD', 'Class', 'DAR', 'CLASS', 'CLASS_STD')
        ...
        current_app.logger.error(f"Error getting darajah unique titles: {e}")
        return 0
    finally:
        if _cur: _cur.close()
        if _conn and _close_conn: _conn.close()
...
def _get_darajah_unique_titles_list(darajah_name: str, start: date, end: date, conn=None):
    """Get list of unique titles issued by a specific darajah."""
    _conn = conn
    _cur = None
    _close_conn = False
    try:
        ...
        _cur.execute("""
            SELECT bib.biblionumber, bib.title, COUNT(*) as issues
            FROM statistics s
            JOIN borrowers b ON s.borrowernumber = b.borrowernumber
            JOIN borrower_attributes ba ON b.borrowernumber = ba.borrowernumber 
                AND ba.code IN ('STD', 'Class', 'DAR', 'CLASS', 'CLASS_STD')
        ...
        current_app.logger.error(f"Error getting darajah unique titles list: {e}")
        return []
    finally:
        if _cur: _cur.close()
        if _conn and _close_conn: _conn.close()
...
def _get_darajah_top_titles_by_language(darajah_name: str, start: date, end: date, lang: str, limit: int = 10, conn=None):
    """Get top titles issued by a specific darajah, filtered by language."""
    _conn = conn
    _cur = None
    _close_conn = False
    try:
        ...
        # Database uses full names like "Arabic", "English", "Lisan-ud-Dawat"
        lang_filter = 'English'
        if lang.lower() == 'arabic':
            lang_filter = 'Arabic'
        elif lang.lower() in ('lisan-ud-dawat', 'lisan', 'lud'):
            lang_filter = 'Lisan-ud-Dawat'
        ...
        query = """
            ...
            LIMIT %s
        """
        params = [start, end, darajah_name, lang_filter, int(limit)]
        ...
        for row in rows:
            ...
            if local_img:
                ...
                row["CoverURL"] = f"/static/images/book-placeholder.png"
                    
        return rows
    except Exception as e:
        current_app.logger.error(f"Error getting darajah top language titles: {e}")
        return []
    finally:
        if _cur: _cur.close()
        if _conn and _close_conn: _conn.close()
...
def _get_darajah_book_review_grades(darajah_name, hijri_year=None):
    """
    Fetch book review grades for all students in a darajah.
    Grades are derived from marks out of 30:
      A  = 25-30  (83%+)
      B  = 20-24  (67-82%)
      C  = 15-19  (50-66%)
      D  = 0-14   (below 50%, but submitted)
      Not Submitted = no record in book_review_marks
    Returns:
      grade_counts: dict {A, B, C, D, Not Submitted}
      student_details: list of dicts with student info and grade
    """
    from services.marks_service import get_conn as get_app_db_conn
    from services.koha_queries import get_ay_bounds

    ay_str = str(hijri_year or Config.CLEAN_ACADEMIC_YEAR())
    conn_app = None

    def _marks_to_grade(marks):
        m = float(marks or 0)
        if m >= 25: return 'A'
        if m >= 20: return 'B'
        if m >= 15: return 'C'
    ...
    try:
        ...
        # Get submitted reviews for this darajah
        cur.execute("""
            SELECT student_trno, student_name, marks, remarks
        ...
        for row in submitted_rows:
            trno = str(row[0] if row[0] else '')
            submitted[trno] = {
                'trno': trno,
                'name': row[1] or f'Student ({trno})',
                'marks': float(row[2] or 0),
                'remarks': row[3] or '',
                'grade': _marks_to_grade(row[2]),
                'submitted': True,
            }

        # Also try matching by student records from all_students in the darajah
        # We'll use the Koha-derived student list to find "Not Submitted"
        ...
        try:
            ...
            cur_k.execute("""
                SELECT DISTINCT
                    ...
                    CONCAT(COALESCE(b.surname, ''), ' ', COALESCE(b.firstname, '')) AS full_name
                FROM borrowers b
                JOIN borrower_attributes cls ON cls.borrowernumber = b.borrowernumber
                    AND cls.code IN ('Class', 'DAR', 'CLASS', 'CLASS_STD')
                    AND cls.attribute = %s
                JOIN borrower_attributes trno ON trno.borrowernumber = b.borrowernumber
                    AND trno.code = 'TRNO'
                WHERE (b.dateexpiry IS NULL OR b.dateexpiry >= CURDATE())
            """, (darajah_name,))
            all_enrolled = cur_k.fetchall()
            cur_k.close()
            conn_k.close()

            for st in all_enrolled:
                trno = str(st.get('trno', '') or '')
                if trno and trno not in submitted:
            ...
            current_app.logger.error(f"Error fetching enrolled students for book review: {e}")

        # Build grade counts
        grade_counts = {'A': 0, 'B': 0, 'C': 0, 'D': 0, 'Not Submitted': len(not_submitted)}
        for s in submitted.values():
            grade_counts[s['grade']] += 1

        # Combine for detail view
        student_details = list(submitted.values()) + not_submitted

        return grade_counts, student_details

    except Exception as e:
        current_app.logger.error(f"Error getting book review grades: {e}")
        return {'A': 0, 'B': 0, 'C': 0, 'D': 0, 'Not Submitted': 0}, []
    finally:
        if conn_app:
            conn_app.close()


def _get_darajah_subject_cloud(darajah_name, start, end, limit=40):
    """
    Fetch subject cloud data for a darajah — ALL books, no language filter.
    ...
    try:
        ...
        query = """
            ...
            FROM statistics s
            JOIN borrowers bor ON s.borrowernumber = bor.borrowernumber
            JOIN borrower_attributes ba ON bor.borrowernumber = ba.borrowernumber
                AND ba.code IN ('STD', 'Class', 'DAR', 'CLASS', 'CLASS_STD')
            JOIN items it ON s.itemnumber = it.itemnumber
            ...
            LIMIT %s
        """
        params = [darajah_name, start, end, int(limit)]
...
# --------------------------------------------------
def _get_ay_student_stats(darajah_name: str, hijri_year=None):
    ...
    try:
        ...
        cur.execute(
            ...
            SELECT 
                ...
                COALESCE(currently_issued_counts.Currently_Issued, 0) AS Currently_Issued,
            ...
            LEFT JOIN borrower_attributes std
                ...
                AND std.code IN ('Class', 'CLASS', 'DAR', 'STD', 'CLASS_STD')
            ...
            LEFT JOIN (
                SELECT i.borrowernumber, COUNT(*) AS Currently_Issued
                FROM issues i
                GROUP BY i.borrowernumber
            ) currently_issued_counts ON currently_issued_counts.borrowernumber = b.borrowernumber
            WHERE s.type = 'issue'
              AND DATE(s.datetime) BETWEEN %s AND %s
            ...
            GROUP BY trno.attribute, b.borrowernumber, b.surname, b.firstname, 
                     fee_totals.FeesPaid_AY, overdue_counts.Overdues, currently_issued_counts.Currently_Issued
        ...
        for student in rows:
            ...
            if issues_ay > 0:
                processed_list.append({
                    ...
                    "FullName": full_name,
                    "Issues_AY": issues_ay,
                    "borrowernumber": student.get("borrowernumber"),
                    "Currently_Issued": int(student.get("Currently_Issued", 0)),
...
# --------------------------------------------------
def _get_all_students_in_darajah(darajah_name: str, hijri_year=None):
    ...
    try:
        ...
        cur.execute(
            ...
            """,
            (start, end, start, end, start, end, darajah_name, darajah_name)
        )
        
        rows = cur.fetchall()
        
        # Fetch Taqeem data from SQLite
        from services.marks_service import get_conn as get_app_db_conn
        taqeem_data = {}
        try:
            conn_app = get_app_db_conn()
            ...
            cur_app.execute("""
                SELECT student_trno, student_username, total_marks, book_issue_total, book_review_marks, program_attendance_marks
                FROM student_taqeem
                WHERE academic_year = ?
            """, (ay_str,))
            
            # handle both tuple and dict cursors
            for t_row in cur_app.fetchall():
                if isinstance(t_row, dict):
                    tr = t_row.get('student_trno')
                    taqeem_data[tr] = t_row
                else:
                    tr = t_row[0]
                    taqeem_data[tr] = {
                        'total_marks': t_row[2],
                        'book_issue_total': t_row[3],
                        'book_review_marks': t_row[4],
                        'program_attendance_marks': t_row[5]
                    }
            conn_app.close()
        except Exception as e:
            current_app.logger.error(f"Error fetching Taqeem for darajah display: {e}")
        ...
        for row in rows:
            ...
            if not full_name or full_name.lower() in ['none', 'null']:
                full_name = f"Student (TR: {tr_no})" if tr_no else "Unnamed Student"
            else:
                full_name = _clean_student_name(full_name)
            
            # Merge Taqeem
            t_info = taqeem_data.get(tr_no, {})
            ...
            students_list.append({
                ...
                "Currently_Issued": int(row.get("Currently_Issued", 0)),
                "Overdues": int(row.get("Overdues", 0)),
                "FeesPaid_AY": float(row.get("FeesPaid_AY", 0.0)),
                "CollectionsUsed": row.get("CollectionsUsed", ""),
                "CardNumber": row.get("cardnumber", ""),
                "Email": row.get("email", ""),
                "DateExpiry": row.get("dateexpiry", ""),
                "TotalMarks": float(t_info.get("total_marks", 0.0)),
                "IssueMarks": float(t_info.get("book_issue_total", 0.0)),
                "ReviewMarks": float(t_info.get("book_review_marks", 0.0)),
                "ProgramMarks": float(t_info.get("program_attendance_marks", 0.0)),
            })
    ...
    try:
        ...
        try:
            ...
            for r in rows:
                ...
                summary_by_trno[tr] = {
                    "borrowernumber": r.get("borrowernumber"),
                    "full_name": full_name,
...
# --------------------------------------------------
def _get_darajah_ay_stats(darajah_name: str, hijri_year=None):
    ...
    try:
        ...
        books_issued = books_row["total_issues"] if books_row else 0
...
# --------------------------------------------------
def _darajah_top_titles_by_lang(darajah_name: str, lang_pattern: str, limit: int = 10, hijri_year=None):
    """Get top titles issued to a darajah in Academic Year by language."""
    start, end = KQ.get_ay_bounds(hijri_year)
    conn = None
    cur = None
    ...
    try:
        ...
        cur.execute(
            ...
            SELECT
                ...
                COUNT(*) AS Times_Issued,
                MAX(DATE(s.datetime)) AS LastIssued
            FROM statistics s
            JOIN borrowers b
                 ON b.borrowernumber = s.borrowernumber
            LEFT JOIN borrower_attributes std
...
# --------------------------------------------------
def _get_top_students_for_darajah(darajah_name: str, limit: int = 10, hijri_year=None):
    """Get top students for a darajah based on issues in Academic Year."""
    start, end = KQ.get_ay_bounds(hijri_year)
    if not start:
        return []
    ...
    try:
        ...
        for row in rows:
            ...
            if not row.get("FullName") or str(row["FullName"]).strip() == "" or str(row["FullName"]).lower() == "none":
                row["FullName"] = f"Student #{row['TRNumber'] or row['cardnumber']}"
            
            row["FullName"] = _clean_student_name(row["FullName"])
            # Ensure display_name includes TR number for "complete name" requirement
...
def dashboard():
    ...
    def _render_empty_dashboard(extra_message=None):
        ...
        return render_template(
            ...
            currently_issued_count=0,
            non_borrowers_count=0,
    ...
    try:
        ...
        # Get accurate AY statistics for specific year
        ay_stats = _get_darajah_ay_stats(darajah_name, hijri_year=hijri_year)
        current_app.logger.info(f"AY stats for {darajah_name}: {ay_stats}")
        
        # Get ALL students in darajah for accurate count
        all_students_list = _get_all_students_in_darajah(darajah_name, hijri_year=hijri_year)
        total_students = len(all_students_list)
        
        # Get ONLY students with books issued
        students_with_books = _get_ay_student_stats(darajah_name, hijri_year=hijri_year)
        ...
        # Get trend data
        trend_labels, trend_values, trend_period_label = _darajah_ay_trend(darajah_name, hijri_year=hijri_year)
        ay_period_label = trend_period_label
        
        # Get top 4 students (Star Patrons)
        top_students = _get_top_students_for_darajah(darajah_name, limit=4, hijri_year=hijri_year)
        
        # Get analytics data
        top_arabic = _get_darajah_top_titles_by_language(darajah_name, start, end, 'Arabic', limit=10)
        top_english = _get_darajah_top_titles_by_language(darajah_name, start, end, 'English', limit=10)
        top_lisan = _get_darajah_top_titles_by_language(darajah_name, start, end, 'Lisan-ud-Dawat', limit=10)
        language_stats = _get_darajah_language_stats(darajah_name, start, end)
        
        # Subject Cloud Data (all languages combined)
        subject_cloud = _get_darajah_subject_cloud(darajah_name, start, end, limit=50)
        unique_titles_count = _get_darajah_unique_titles_count(darajah_name, start, end)
        unique_titles_list = _get_darajah_unique_titles_list(darajah_name, start, end)
        
        # Use single connection for manual queries
        from db_koha import get_koha_conn
        conn_k = None
        try:
            conn_k = get_koha_conn()
            cur_k = conn_k.cursor(dictionary=True)
            ...
            cur_k.execute("""
                ...
                JOIN borrower_attributes ba ON b.borrowernumber = ba.borrowernumber 
                    AND ba.code IN ('Class', 'CLASS', 'DAR', 'STD', 'CLASS_STD')
                WHERE ba.attribute = %s
            """, (darajah_name,))
            res_ci = cur_k.fetchone()
            currently_issued_count = res_ci['currently_issued'] if res_ci else 0
            
            # Teacher Profile 
            cur_k.execute("""
                SELECT surname, borrowernumber 
                FROM borrowers 
                WHERE userid = %s
                LIMIT 1
            """, (username,))
            teacher_prof = cur_k.fetchone()
            cur_k.close()
            
            teacher_surname = teacher_prof['surname'] if teacher_prof else username
            teacher_borrowernumber = teacher_prof['borrowernumber'] if teacher_prof else None
        finally:
            if conn_k:
                conn_k.close()
        ...
        # Book Review stats
        review_grade_counts, review_details = _get_darajah_book_review_grades(darajah_name, hijri_year)
        ...
        for tr_no, month_data in month_summary.items():
            ...
            month_students_list.append({
                ...
                "borrowernumber": month_data["borrowernumber"],
        ...
        # KPIs calculations
        non_borrowers_count = max(0, total_students - active_borrowers)
        
        darajah_masool_name = teacher_surname
        today = date.today()
        current_month_label = _hijri_month_year_label(today)
        ay_period_label = get_academic_year_period(hijri_year=hijri_year)
        
        # Prepare students list with display names
        # Calculate Mustawa target for this darajah
        target = 0
        try:
            m = re.search(r"(\d+)", str(darajah_name))
            if m:
                d_num = int(m.group(1))
                if 1 <= d_num <= 2: target = 3
                elif 3 <= d_num <= 4: target = 4
                elif 5 <= d_num <= 7: target = 5
                elif 8 <= d_num <= 11: target = 6
        except:
            target = 0
        ...
        for student in all_students_list:
            tr_no = str(student.get("TRNumber") or "")
            issues_ay = int(student.get("Issues_AY", 0))
            
            # Non-Borrower check (AY)
            student["IsNonBorrower"] = (issues_ay == 0)
            
            # Mustawa Achievement check (Using AY count to match teacher's visual expectation)
            # This ensures that if they see 3 or 6 books, the card is green.
            student["MustawaAchieved"] = (target > 0 and issues_ay >= target)
            ...
            if "Currently_Issued" not in student: student["Currently_Issued"] = 0
            if "Issues_AY" not in student: student["Issues_AY"] = 0

        # Sort students by Issues_AY (highest to lowest) explicitly
        all_students_list.sort(key=lambda x: x.get("Issues_AY", 0), reverse=True)
        ...
        # Ensure subject_cloud is safe for render
        if not subject_cloud: subject_cloud = []

        current_app.logger.info(f"DEBUG: Rendering teacher dashboard. darajah={darajah_name}, target={target}, students={len(all_students_list)}")
        ...
        return render_template(
            ...
            teacher_borrowernumber=teacher_borrowernumber or 0,
            ...
            currently_issued_count=currently_issued_count,
            non_borrowers_count=non_borrowers_count,
            ...
            review_grade_counts=review_grade_counts,
    ...
    for darajah_info in all_darajahs:
        ...
        try:
            ...
            darajahs_with_stats.append({
                ...
                "gender_code": parsed["gender_code"],
                "icon": icon,
                "ActiveStudents": ay_stats["active_students"],
                "TotalStudents": ay_stats["total_students"],
