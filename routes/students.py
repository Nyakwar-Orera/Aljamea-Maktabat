...
def patron_photo(borrowernumber):
    ...
    try:
        ...
        row = cur.fetchone()
        if row and row['imagefile']:
            return send_file(BytesIO(row['imagefile']), mimetype=row.get('mimetype', 'image/jpeg'))
    except Exception as e:
        current_app.logger.error(f"Error fetching photo: {e}")
    finally:
        try:
...
def _ay_period_label():
    ...
    return f"{KQ.get_hijri_month_year_label(start)} to {KQ.get_hijri_month_year_label(end)}"
...
def get_student_info(identifier):
    ...
    try:
        ...
        # Fetch ALL currently issued books (Active Loans) — regardless of date
        cur.execute(
            """
            SELECT
                bi.biblionumber,
        ...
        # Fetch past issues (Statistics) — only for current Academic Year
        cur.execute(
            """
            SELECT
                bi.biblionumber,
                ...
                s.datetime AS date_issued,
                (
                    SELECT iss.date_due FROM issues iss 
                    WHERE iss.itemnumber = s.itemnumber AND iss.borrowernumber = s.borrowernumber
                    LIMIT 1
                ) AS date_due,
                (
                    SELECT MAX(return_stat.datetime) FROM statistics return_stat
                    WHERE return_stat.itemnumber = s.itemnumber 
                    AND return_stat.borrowernumber = s.borrowernumber
                    AND return_stat.type = 'return'
                    AND return_stat.datetime >= s.datetime
                ) AS returndate,
                1 AS returned
            FROM statistics s
            JOIN items it USING (itemnumber)
        ...
        past_issues = cur.fetchall()

        # Combine them: Active Loans take priority, Past Issues provide historical context for the AY
        # Use biblionumber + date_issued as a unique key to prevent duplicates if something is in both
        borrowed_books = active_loans + past_issues
        borrowed_books.sort(key=lambda x: x["date_issued"] or datetime.min, reverse=True)
    ...
    # Photo lookup via database
    pi_conn = None
    try:
        pi_conn = get_koha_conn()
        cur_pi = pi_conn.cursor()
        cur_pi.execute("SELECT 1 FROM patronimage WHERE borrowernumber=%s", (borrowernumber,))
        if cur_pi.fetchone():
            student["Photo_URL"] = url_for("students.patron_photo", borrowernumber=borrowernumber)
    except Exception as e:
        current_app.logger.error(f"Error checking patronimage: {e}")
    finally:
        try:
            if 'cur_pi' in locals() and cur_pi:
                cur_pi.close()
        except Exception:
            pass
        try:
            if 'pi_conn' in locals() and pi_conn:
                pi_conn.close()
        except Exception:
            pass
    ...
    if student_identifier:
        try:
            current_ay = Config.CURRENT_ACADEMIC_YEAR().replace('H', '').strip()
    ...
    # Calculate Subject of Interest (from MARC 650$a)
    subject_map = {}
    for b in borrowed_books:
        sub_str = b.get("subject")
        if sub_str:
            # Koha's ExtractValue on multiple fields typically returns them joined.
            # We'll split on common separators if applicable, but usually 650$a are terms.
            # Let's clean it up a bit (strip whitespace).
            subjects = [s.strip() for s in sub_str.split('|') if s.strip()] # Koha often uses | or space 
            if not subjects:
                subjects = [sub_str.strip()]
            
            for s in subjects:
                subject_map[s] = subject_map.get(s, 0) + 1
    
    # Sort subjects by frequency and take top 5
    sorted_subjects = sorted(subject_map.items(), key=lambda x: x[1], reverse=True)
    subject_of_interest = [s[0] for s in sorted_subjects[:5]]

    # Current Hijri Month Issue Count
    current_month_count = 0
    if grouped_sorted:
        # The first item in grouped_sorted is the most recent month
        current_month_count = len(grouped_sorted[0][1])
    ...
    student["SubjectOfInterest"] = subject_of_interest
    ...
    student["Metrics"] = {
        ...
        "ProgramParticipationCount": len(student.get("ProgramParticipation", [])),
...
@bp.route("/portal")
def student_portal():
    """Personalized portal for the logged-in student."""
    if not session.get("logged_in") or session.get("role") != "student":
        flash("❌ Access denied. Please log in as a student.", "danger")
        return redirect(url_for("auth_bp.login"))

    # Identifier can be cardnumber or ITS ID from session
    identifier = session.get("cardnumber") or session.get("username")
    if not identifier:
        flash("❌ Student identity not found in session.", "danger")
        return redirect(url_for("auth_bp.login"))
    ...
    recs = {
        "trending": RecommendationService.get_marhala_recommendations(marhala, limit=4),
        "personalized": RecommendationService.get_personalized_recommendations(identifier, limit=4)
    }
...
def export_student_pdf(identifier, orientation='portrait'):
    ...
    summary_data = {
        ...
        # Added KPI metrics for dashboard-style PDF
        'Currently Issued': metrics.get('CurrentlyIssued', 0),
        'Past Due Date': metrics.get('OverdueNow', 0),
        'Monthly Target': metrics.get('MaxBooksAllowed', 0),
        'Outstanding Fines': f"{metrics.get('OutstandingBalance', 0.0):.2f}",
        'Programs Attended': metrics.get('ProgramParticipationCount', 0),
        'Subjects Profile': ", ".join(info.get('SubjectOfInterest', []))
    }
