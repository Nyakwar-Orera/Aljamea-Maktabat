...
# --------------------------------------------------
def _get_accurate_marhala_stats(marhala_code):
    """
    ...
    try:
        ...
        cur.execute(
            ...
            SELECT 
                ...
                COUNT(DISTINCT trno.attribute) as borrowers_with_tr
            FROM borrowers b
            LEFT JOIN borrower_attributes trno
                 ON trno.borrowernumber = b.borrowernumber
                AND trno.code = 'TRNO'
...
# --------------------------------------------------
def get_marhala_top_titles(marhala_code: str, lang_code: str, limit: int = 10):
...
# --------------------------------------------------
def get_marhala_ay_trend(marhala_code: str):
    """Monthly borrowing trend for the marhala over the current Academic Year."""
    try:
        labels, values = KQ.get_ay_trend_data(marhala_code=marhala_code)
...
def dashboard():
    ...
    if not marhala_name:
        if role == "admin":
            ...
            # Calculate totals from all marhalas
            for marhala in all_marhalas:
                total_currently_issued += marhala.get("currently_issued", 0)
                total_fees += marhala.get("ay_fees", 0)
                total_issues += marhala.get("ay_issues", 0)
            ...
            return render_template(
                ...
                ay_period=ay_period,
                total_currently_issued=total_currently_issued,
                total_fees=total_fees,
                total_issues=total_issues,
                total_marhalas=total_marhalas,
                marhalas_with_data=len(marhalas_with_data),
                time_period=_get_time_period_label(),
                format_currency=_format_currency
            )
        else:
    ...
    all_marhalas_list = get_all_marhalas_for_hod()
    ...
    # Get detailed stats
    total_borrowers, active_borrowers, total_issues_ay, total_fees_ay, currently_issued, overdues_now = _get_accurate_marhala_stats(marhala_code)
    ...
    # Get trend data
    trend_labels, trend_values, trend_period_label = get_marhala_ay_trend(marhala_code)
    ...
    top_english = []
    if stats_available:
        top_arabic = get_marhala_top_titles(marhala_code, lang_code="ar", limit=5)
        top_english = get_marhala_top_titles(marhala_code, lang_code="eng", limit=5)
    ...
    # Placeholder for recent_activities and overdue_books_list
    recent_activities = []
    overdue_books_list = []
